					...
					if (credor == false) {
						...
						if (aluno == false) {
							...
							insertAluno(credotAtual, alunoId, alunoNome,
									...
									credorNome, codEmp, cursoId,turmaId);
						...
						insertLogIntegracao("Credor ja cadastrado", "Aviso",
								credorNome, "");
						...
						if (aluno == false) {
							...
							insertAluno(credorCadastrado, alunoId, alunoNome,
									...
									alunoEmail, alunoSituacao, alunoSituacaoId,
									credorNome, codEmp, cursoId,turmaId);
							insertLogIntegracao("Aluno ja Cadastro ", "Aviso",
									"", alunoNome);
						}
...
//	}
	public BigDecimal insertCursoProj(String cursoDescricao, String credorNome,
	        String alunoNome, BigDecimal codEmp, String cursoId) throws Exception {
	    ...
	    try {
	        ...
	        if (validarCadastroCursoProj(cursoDescricao, credorNome, alunoNome)) {
	            ...
	            System.out.println("Curso já cadastrado: " + cursoDescricao);
	        }
...
//	}
	...
	public void insertAluno(BigDecimal credotAtual, String alunoId,
	        ...
	        String alunoResidencial, String alunoEmail, String alunoSituacao,
	        String alunoSituacaoId, String credorNome, BigDecimal codEmp,
	        String cursoId, String turmaId) throws Exception { // Adicionado turmaId como parâmetro
		
		
		System.out.println("[DEBUG] Iniciando insertAluno...");
	    System.out.println("[DEBUG] turmaId recebido: " + turmaId);

	    if (credotAtual == null) {
	        throw new Exception("CODPARC não pode ser nulo.");
	    }
	    ...
	    try {
	        ...
	        // Incluindo a coluna TURMA na instrução SQL
	        String sqlP = "INSERT INTO AD_ALUNOS (CODPARC, ID_EXTERNO, NOME, NOME_SOCIAL, ENDERECO, "
	                + "CEP, BAIRRO, CIDADE, UF, SEXO, DATA_NASCIMENTO, RG, CPF, TELEFONE_CELULAR, TELEFONE_RESIDENCIAL, "
	                + "EMAIL, SITUACAO, SITUACAO_ID, CODEMP, CODCENCUS, TURMA) " // Botei turma aqui 
	                + "VALUES "
	                + "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
	                + "(SELECT TO_CHAR(TO_DATE(?, 'yyyy-MM-dd'), 'dd/MM/yyyy') FROM dual), "
	                + "?, ?, ?, ?, ?, ?, ?, ?, (SELECT CODCENCUS FROM TSICUS WHERE AD_IDEXTERNO = ?), ?)"; // Adicionado parâmetro para TURMAaqui
	        ...
	        pstmt.setBigDecimal(19, codEmp);
	        pstmt.setString(20, cursoId);
	        pstmt.setString(21, turmaId); // Passando o turmaId para a coluna TURMA
	        
	        
	        
	        System.out.println("TURMA: " + turmaId);
	        
	        
	        pstmt.executeUpdate();
	        
	        
	        System.out.println("[DEBUG] Aluno inserido com sucesso. Turma: " + turmaId);

	    } catch (SQLException e) {
	    	System.err.println("[DEBUG] Erro ao inserir aluno: " + e.getMessage());
