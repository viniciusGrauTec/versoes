...
public class acaoGetTituloFornecedorTiposCarga implements AcaoRotinaJava, ScheduledAction{
	...
	public void onTime(ScheduledActionContext arg0) {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println("ENTROU NO OnTime DA  acaoGetTituloFornecedorTiposCarga! ");
				
				selecionarTiposFornecedores(jdbc, mapaInfConta, mapaInfConta, query, query, codEmp, mapaInfConta, mapaInfFinanceiroBaixado, mapaInfFinanceiroBanco, mapaInfConta, mapaInfConta, mapaInfConta, mapaInfConta, mapaInfConta, mapaInfConta, query);
	...
	public void selecionarTiposFornecedores(JdbcWrapper jdbc, Map<String, BigDecimal> mapaInfIdParceiros,
			Map<String, BigDecimal> mapaInfParceiros, String url, String token, BigDecimal codEmp,Map<String, BigDecimal> mapaInfCenCusEmp,Map<BigDecimal, String> mapaInfFinanceiroBaixado,
			Map<BigDecimal, BigDecimal> mapaInfFinanceiroBanco, Map<String, BigDecimal> mapaInfNatureza,  Map<String, BigDecimal> mapaInfBanco,  Map<String, BigDecimal> mapaInfConta,
			Map<String, BigDecimal> mapaInfCenCus,  Map<String, BigDecimal> mapaInfFinanceiro, Map<String, BigDecimal> mapaInfNaturezaEmp, String tipoEmpresa)
 {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println("ENTROU NO METODO DA  acaoGetTituloFornecedorTiposCarga! ");
				
				String urlComTipoFornecedor = url + "&fornecedor_tipo=" + descricao;

				// Chama o método iterarEndpoint para cada tipo de fornecedor
				 String tipoEmpresaTrimmado = (tipoEmpresa != null) ? tipoEmpresa.trim() : "";


			    iterarEndpoint(tipoEmpresaTrimmado, mapaInfNaturezaEmp, mapaInfCenCusEmp,
			            mapaInfFinanceiroBanco, mapaInfFinanceiroBaixado, 
			            mapaInfNatureza, mapaInfBanco, mapaInfConta, 
			            mapaInfFinanceiro, mapaInfCenCus, 
			            mapaInfParceiros, url, token, codEmp);
	...
	public void iterarEndpoint(String tituloAberto,
			...
			String dataFim, String idForn) throws Exception {
			...
			while (!atual.isAfter(fim)) {
				...
				if(tituloAberto.equalsIgnoreCase("S")){
					...
					if(idForn != null && !idForn.isEmpty()){
						...
						response = apiGet(url+ "/financeiro/clientes/titulos-pagar?quantidade=0"
	...
	//SEMPRE O SEGUNDO ITERARQUE VAI SER USADO POR TER UMA UNICA DATA
