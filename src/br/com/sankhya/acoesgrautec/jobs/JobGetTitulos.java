...
public class JobGetTitulos implements ScheduledAction {
	...
	public void onTime(ScheduledActionContext arg0) {
		...
		try {
			...
			while (rs.next()) {
				...
				iterarEndpoint(mapaInfFinanceiro, mapaInfRecDesp, mapaInfConta,
			...
			System.out
					.println("Finalizou o financeiro dos alunos carga empresa 3");
	...
	public void iterarEndpoint(Map<String, BigDecimal> mapaInfFinanceiro,
		...
		try {
			...
			// break;
			// }
			cadastrarFinanceiro(mapaInfFinanceiro, mapaInfRecDesp,
	...
	public void cadastrarFinanceiro(Map<String, BigDecimal> mapaInfFinanceiro,
		...
		try {
			 ...
			 * 
			 * //String sqlP =
			 * "SELECT ID_EXTERNO, CODPARC, CODCENCUS, INTEGRADO FROM (SELECT ID_EXTERNO, CODPARC, CODCENCUS, INTEGRADO FROM AD_ALUNOS WHERE CODEMP = "
			 * +codemp+
			 * " AND NVL(INTEGRADO, 'N') = 'N' ORDER BY ID_EXTERNO, CODPARC DESC ) WHERE ROWNUM <= 300"
			 * ; //AND ROWNUM <= 60"; String sqlP =
			 * "SELECT ID_EXTERNO, CODPARC, CODCENCUS, INTEGRADO FROM AD_ALUNOS WHERE ID_EXTERNO = '"
			 * + matricula + "' AND CODEMP = " + codemp; //AND ROWNUM <= 60"; //
					...
					if (!situacao_titulo.equalsIgnoreCase("X")
						...
						if (codCenCus.compareTo(BigDecimal.ZERO) == 0) {
							codCenCus = Optional.ofNullable(
									mapaInfCenCus.get(idAluno)).orElse(
									BigDecimal.ZERO);
						}
						...
						if (validarDataLimite(dtPedido)
							...
							System.out
									.println("Data pedido inferior a data limite");
						}
